...
@Component({
  ...
  templateUrl: './edit-marca.html',
  styleUrl: './edit-marca.css',
})
export class EditMarca implements OnInit{
...
private RegisterNew(datos :BasicModel) {
    ...
    this.marcaService.register(datos)
           .subscribe({
...
private Update(datos :BasicModel) {
    ...
    this.marcaService.update(datos)
           .subscribe({
              next: (data: ResponseData) => {
                this.isLoading = false;
              console.log('Success payload:', data);
...
async safeCall(id: string) {
      ...
      this.marcaService.getmarca(id).subscribe({
        ...
        error: (err) => {
          console.error("Error reading marcas");
        }
